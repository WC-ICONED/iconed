(() => {
  ...
  function renderSharePreview(puzzleNum) {
    ...
    line3.textContent = "https://www.iconed.wtf/fck/";
  ...
  function buildExportCode() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k.startsWith("fck-")) data[k] = localStorage.getItem(k);
    }
  ...
  function importStats() {
    ...
    try {
      ...
      const keys = Object.keys(data).filter(k => k.startsWith("fck-"));
  ...
  function getCachedThumb(playerId) {
    return localStorage.getItem(`fck-thumb-${playerId}`);
  }

  function setCachedThumb(playerId, src) {
    try { localStorage.setItem(`fck-thumb-${playerId}`, src); } catch {}
  }
  ...
  function cleanupCorruptedArchive() {
    ...
    for (let d = 0; d < ALBUM_TOTAL; d++) {
      ...
      if (!validIds.has(archived.puzzleId)) {
        localStorage.removeItem(`fck-archive-${d}`);
        cleaned++;
      }
    }
    if (cleaned > 0) {
  ...
  function initConsent() {
    ...
    const consent = localStorage.getItem("fck-consent");
    if (consent) {
      overlay.classList.add("hidden");
    }

    acceptBtn.addEventListener("click", () => {
      localStorage.setItem("fck-consent", "accepted");
      overlay.classList.add("hidden");
    });

    declineBtn.addEventListener("click", () => {
      localStorage.setItem("fck-consent", "declined");
      overlay.classList.add("hidden");
    });
