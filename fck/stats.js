window.Stats = (() => {
  ...
  function getStats(mode) {
    const raw = localStorage.getItem(key("fck-stats", mode));
  ...
  function saveStats(stats, mode) {
    localStorage.setItem(key("fck-stats", mode), JSON.stringify(stats));
  }
  ...
  function resetStats(mode) {
    localStorage.removeItem(key("fck-stats", mode));
  }

  function getDayState(mode) {
    const raw = localStorage.getItem(key("fck-day-state", mode));
    if (!raw) return null;
    try { return JSON.parse(raw); } catch { return null; }
  }

  function setDayState(state, mode) {
    localStorage.setItem(key("fck-day-state", mode), JSON.stringify(state));
  }

  function clearDayState(mode) {
    localStorage.removeItem(key("fck-day-state", mode));
  }

  function getArchiveState(dayNum) {
    const raw = localStorage.getItem(`fck-archive-${dayNum}`);
    if (!raw) return null;
    try { return JSON.parse(raw); } catch { return null; }
  }

  function setArchiveState(dayNum, state) {
    localStorage.setItem(`fck-archive-${dayNum}`, JSON.stringify(state));
  }
