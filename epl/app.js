(() => {
  ...
  function renderClues() {
    ...
    if (wrong >= 3) {
      ...
      if (puzzle.shirtNumber == null) {
        els.clueShirt.textContent = puzzle.shirtNumberNote ? "N/A" : "Unknown";
      } else {
        els.clueShirt.textContent = `#${puzzle.shirtNumber}`;
      }
    }
  ...
  function revealFunFact() {
    if (puzzle && puzzle.funFact && els.funFact) {
      els.funFact.textContent = puzzle.funFact;
  ...
  function buildShareText(puzzleNum) {
    ...
    return `ICONED·PL №${puzzleNum} — ${puzzle.season} 🦁 ${flag}\n⚽ ${cells.join(" ")}\nhttps://www.iconed.wtf/epl/`;
  }

  function renderSharePreview(puzzleNum) {
    const flag = FLAG[puzzle.nationality] || "";
    const line1 = document.createElement("div");
    line1.textContent = `ICONED·PL №${puzzleNum} — ${puzzle.season} 🦁 ${flag}`;
    ...
    line3.textContent = "https://www.iconed.wtf/epl/";
  ...
  function finish(won) {
    ...
    if (els.funFact) els.funFact.classList.add("hidden");
    hideSuggestions();
  ...
  function buildExportCode() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k.startsWith("epl-")) data[k] = localStorage.getItem(k);
    }
  ...
  function importStats() {
    ...
    try {
      ...
      const keys = Object.keys(data).filter(k => k.startsWith("epl-"));
  ...
  function getCachedThumb(playerId) {
    return localStorage.getItem(`epl-thumb-${playerId}`);
  }

  function setCachedThumb(playerId, src) {
    try { localStorage.setItem(`epl-thumb-${playerId}`, src); } catch {}
  }
  ...
  function initConsent() {
    ...
    const consent = localStorage.getItem("epl-consent");
    if (consent) {
      overlay.classList.add("hidden");
    }

    acceptBtn.addEventListener("click", () => {
      localStorage.setItem("epl-consent", "accepted");
      overlay.classList.add("hidden");
    });

    declineBtn.addEventListener("click", () => {
      localStorage.setItem("epl-consent", "declined");
      overlay.classList.add("hidden");
    });
