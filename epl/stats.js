window.Stats = (() => {
  ...
  function getStats(mode) {
    const raw = localStorage.getItem(key("epl-stats", mode));
  ...
  function saveStats(stats, mode) {
    localStorage.setItem(key("epl-stats", mode), JSON.stringify(stats));
  }
  ...
  function resetStats(mode) {
    localStorage.removeItem(key("epl-stats", mode));
  }

  function getDayState(mode) {
    const raw = localStorage.getItem(key("epl-day-state", mode));
    if (!raw) return null;
    try { return JSON.parse(raw); } catch { return null; }
  }

  function setDayState(state, mode) {
    localStorage.setItem(key("epl-day-state", mode), JSON.stringify(state));
  }

  function clearDayState(mode) {
    localStorage.removeItem(key("epl-day-state", mode));
  }

  function getArchiveState(dayNum) {
    const raw = localStorage.getItem(`epl-archive-${dayNum}`);
    if (!raw) return null;
    try { return JSON.parse(raw); } catch { return null; }
  }

  function setArchiveState(dayNum, state) {
    localStorage.setItem(`epl-archive-${dayNum}`, JSON.stringify(state));
  }
