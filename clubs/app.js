(() => {
  ...
  // Per-club config — set in each club's index.html before this script loads
  ...
  const els = {
    ...
    funFactSlot: document.getElementById("funFactSlot"),
    funFactSlotText: document.getElementById("funFactText"),
    funFactPlaceholder: document.getElementById("funFactPlaceholder"),
  ...
  function revealFunFactSlot() {
    if (!els.funFactSlot || !puzzle || !puzzle.funFact) return;
    if (els.funFactSlot.classList.contains("locked")) {
      els.funFactSlot.classList.remove("locked");
      els.funFactSlot.classList.add("revealed");
      els.funFactSlot.classList.add("just-unlocked");
      setTimeout(() => els.funFactSlot.classList.remove("just-unlocked"), 600);
    }
    if (els.funFactSlotText) {
      els.funFactSlotText.textContent = puzzle.funFact;
    }
  }
  ...
  async function maybeFetchImage() {
    ...
    // Try Wikipedia; fall back to the shadow silhouette on miss.
  ...
  function buildShareText(puzzleNum) {
    ...
    return `ICONED·${cfg.shareLabel} №${puzzleNum} — ${puzzle.season} ${cfg.countryFlag} ${flag}\n⚽ ${cells.join(" ")}\nhttps://www.iconed.wtf/${cfg.urlPath}/`;
  }

  function renderSharePreview(puzzleNum) {
    const flag = FLAG[puzzle.nationality] || "";
    const line1 = document.createElement("div");
    line1.textContent = `ICONED·${cfg.shareLabel} №${puzzleNum} — ${puzzle.season} ${cfg.countryFlag} ${flag}`;
    ...
    line3.textContent = `https://www.iconed.wtf/${cfg.urlPath}/`;
  ...
  function renderPlayerCard() {
    ...
    els.portraitMeta.textContent = `${puzzle.nationality.substring(0, 3).toUpperCase()} · ${puzzle.season}`;

    els.playerTags.innerHTML = "";
    const tags = [];
    const POS_ABBR = { "Goalkeeper": "GK", "Defender": "DEF", "Midfielder": "MID", "Forward": "FWD" };
    if (puzzle.position) tags.push(POS_ABBR[puzzle.position] || puzzle.position.substring(0, 3).toUpperCase());
    if (puzzle.shirtNumber != null) tags.push(`#${puzzle.shirtNumber}`);
    if (puzzle.club) tags.push(puzzle.club);
    tags.push(`${puzzle.season}`);
  ...
  function resetStickerStates() {
    // Shirt is now an initial clue — don't lock it.
    els.stickerAge.classList.add("locked");
    els.stickerPosition.classList.add("locked");
    if (els.funFactSlot) {
      els.funFactSlot.classList.add("locked");
      els.funFactSlot.classList.remove("revealed");
    }
  ...
  function buildExportCode() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k.startsWith(`${cfg.slug}-`)) data[k] = localStorage.getItem(k);
    }
  ...
  function importStats() {
    ...
    try {
      ...
      const keys = Object.keys(data).filter(k => k.startsWith(`${cfg.slug}-`));
  ...
  function getCachedThumb(playerId) {
    return localStorage.getItem(`${cfg.slug}-thumb-${playerId}`);
  }

  function setCachedThumb(playerId, src) {
    try { localStorage.setItem(`${cfg.slug}-thumb-${playerId}`, src); } catch {}
  }
  ...
  function buildAlbumSlot(dayNum, status) {
    ...
    slot.innerHTML = `
      ...
      <div class="album-slot-meta">
        ...
        <span class="album-slot-detail">${flag} WC ${p.season}</span>
      </div>
  ...
  function cleanupCorruptedArchive() {
    ...
    for (let d = 0; d < ALBUM_TOTAL; d++) {
      ...
      if (!validIds.has(archived.puzzleId)) {
        localStorage.removeItem(`${cfg.slug}-archive-${d}`);
        cleaned++;
      }
    }
    if (cleaned > 0) {
  ...
  function initConsent() {
    ...
    const consent = localStorage.getItem(`${cfg.slug}-consent`);
    if (consent) {
      overlay.classList.add("hidden");
    }

    acceptBtn.addEventListener("click", () => {
      localStorage.setItem(`${cfg.slug}-consent`, "accepted");
      overlay.classList.add("hidden");
    });

    declineBtn.addEventListener("click", () => {
      localStorage.setItem(`${cfg.slug}-consent`, "declined");
      overlay.classList.add("hidden");
    });
